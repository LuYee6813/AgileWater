...
const getWaterStationBySn = async (req, res) => {
  ...
  // 驗證 sn 是否為有效的整數
  if (!/^\d+$/.test(sn)) {
      return res.status(400).json({ error: 'Invalid SN. SN must be an integer.' });
  }
...
const getNearbyWaterStations = async (req, res) => {
  ...
  try {
      ...
      if (result.rows.length === 0) {
          return res.status(404).json({ message: 'No water stations found within the specified radius.' });
      }
